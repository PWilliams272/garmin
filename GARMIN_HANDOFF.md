...
- Standalone viewer app: `garmin.peterwilliams.dev`, live as of 2026-08-01 -- same shared EC2 host as `kaya`/`aws_monitor` (`3.16.167.114`), code at `/home/ubuntu/garmin_viewer`, `garmin-viewer.service` (gunicorn, port `8030`, `127.0.0.1` only), nginx vhost `/etc/nginx/sites-available/garmin-viewer` with **HTTP Basic Auth** (`/etc/nginx/.htpasswd-garmin-viewer`) -- deliberately not the main-site shared-session `auth_request` pattern, same reasoning as `aws_monitor`: that pattern depends on `SESSION_COOKIE_DOMAIN`, which was removed 2026-07-28 after breaking main-site login and hasn't had its root cause investigated (see `system-overview/AGENT_HANDOFF.md`). Reuses the existing Flask app as-is (not a FastAPI rewrite, despite `WEB_APP_SETUP.md` describing FastAPI -- a deliberate deviation to avoid re-templating already-built, tested Jinja pages; gunicorn instead of uvicorn as the only infra consequence).
  ...
  - Reads S3 only: `GARMIN_VIEWER_SOURCE=s3` (plus `GARMIN_S3_BUCKET=my-garmin-data`, `GARMIN_RUNTIME_ENV=aws`) set via the systemd unit's `Environment=` lines, which `garmin/app/routes.py`'s `DEFAULT_SOURCE` reads so every route defaults to S3 without needing `?source=s3` on every request. AWS credentials come from the host's existing instance profile (`ec2-kaya-viewer-role`, extended additively with a new `garmin-viewer-readonly` inline policy granting `s3:GetObject` on `my-garmin-data/curated/*` and `/viewer_cache/*` -- `s3:ListBucket` was already covered by `aws-monitor-readonly`'s wildcard prefix) -- **never create a competing instance profile**, this host only supports one at a time.
...
- Analyzer Lambda's CI deploy (`deploy-lambda-analyzer.yml`) currently fails: the GitHub Actions IAM user (`garmin-app`) isn't authorized for `ecr:GetAuthorizationToken`. Not yet fixed as of 2026-08-01 -- the analyzer's actual deployed image stays current via the manual `docker buildx build --push` + `aws lambda update-function-code` path (see Useful Commands below), so this doesn't block the pipeline, just automated redeploys of that one Lambda.
